...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, status, status_detail, transaction_amount, payment_type_id, point_of_interaction } = payment
    ...
    if (status === "pending" && payment_type_id === "bank_transfer") {
      ...
      const pixQrCodeBase64 = point_of_interaction?.transaction_data?.qr_code_base64
